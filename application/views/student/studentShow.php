...
if (empty($examSchedule)) {
	...
	$gradeTest = ''; $i = 0; $globalI;
    foreach ($examSchedule as $key => $value) {
        ?>
										<a class = "pull-right" href = "<?=base_url()?>admin/subjectassign/pdf/<?php echo $id; ?>" style="color:#fff;margin-top: 7px;margin-right: 5px;"><i class = "fa fa-file-pdf-o"></i> PDF & <i class = "fa fa-print"></i> Print</a>
				...
				<td>
					<?php
						if($globalI == strlen($gradeTest)){
							echo $this->lang->line('grand_total') . ": " ."N/A";
						}else{
						...
						if (!empty($gradeList)) {
							foreach ($gradeList as $key => $value) {
								if ($foo >= $value['mark_from'] && $foo <= $value['mark_upto']) {
									echo $this->lang->line('grade') . " : " . $value['name'];
									break;
								}
							}
						}
                                                                ...
                                                                <h5 class="description-header"><?php echo $this->lang->line('percentage'); ?>:
                                                                    <span class="description-text">
																		...
																		if($globalI == strlen($gradeTest)){
																			...
																			if(json_decode(json_encode($siblings_classname), True)[0]['class'] == '10th'){
																				$foo = ($obtain_marks * 100) / $total_marks;
