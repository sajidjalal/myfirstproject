            ...
            if (!empty($examSchedule)) {
                ...
                foreach ($studentList as $stu_key => $stu_value) {
                    ...
                    foreach ($examSchedule as $ex_key => $ex_value) {
						...
						array_push($array['type'], $student_exam_result->type);
                        $x[] = $exam_array;
                    }
                    if (empty($x)) {
            ...
            if (!empty($examSchedule)) {
                ...
                foreach ($studentList as $stu_key => $stu_value) {
                    ...
                    foreach ($examSchedule as $ex_key => $ex_value) {
                        ...
                        $exam_array['attendence'] = $student_exam_result->attendence;
                        $exam_array['get_marks'] = $student_exam_result->get_marks;
						$exam_array['type'] = $student_exam_result->type;
                        $x[] = $exam_array;
                    }
                    $array['exam_array'] = $x;
                    $new_array[] = $array;
				...
				$gradelist = $this->subjectassign_model->getgrademark($class_id);
				$subjectlist = $this->subjectassign_model->getsubjectbyid($class_id);
            ...
            if ($this->input->post('save_exam') == "save_exam") {
                ...
                foreach ($student_array as $key => $student) {
                    foreach ($exam_array as $key => $exam) {
						...
						$record['type'] = $this->input->post('option' . $student . "_" . $exam);
